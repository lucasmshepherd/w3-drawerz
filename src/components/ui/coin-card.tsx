...
export function CoinCard({
  ...
  return (
    <div
      ...
      style={
        name == 'Ethereum'
          ? { backgroundColor: color, color: '#010101' }
          : { backgroundColor: color, color: '#010101' }
      }
    >
      <h4 className="mb-8 text-sm font-medium uppercase tracking-wider">
        {name}
      </h4>
      <div className="relative h-20 lg:h-24 xl:h-28 3xl:h-36">
        <Image src={logo} alt={name} height={90} priority />
      </div>
      <div className="mb-2 mt-8 text-sm font-medium tracking-wider lg:text-lg 2xl:text-xl 3xl:text-2xl">
        {balance}
        <span className="uppercase"> {symbol}</span>
      </div>
      <div className="flex items-center justify-between text-xs font-medium 2xl:text-sm">
        <span className="tracking-wider">{usdBalance}</span>
        <span className="flex items-center">{change}</span>
      </div>
    </div>
  );
}
